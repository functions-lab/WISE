...
class SA_Generic(Instrument):
    ...
    def enable_marker(self, num):                                               # enable marker (num)
        """ enable marker (num) """
        self.write("CALC:MARK%d NORM" % num)
        return
    ...
    def set_marker_peak(self, num):                                             # set marker to peak
        """ set marker to peak """
        self.write("CALC:MARK%d:FUNC:MAX" % num)
        return
    ...
    def get_marker(self, num):                                                  # read value of marker
        ...
        x = str(self.query("CALC:MARKER%d:Y?" % num))
        return x.split(",")[0]
    ...
    # def set_avg_type(self, type):                                               # set AVG type
    #     """ set AVG type """
    #     if type not in ["AUTO","POW","LOG","VOLT"]:
    #         raise Exception("invalid average type.")
    #     self.write("AVER:TYPE " + type)
    #     return

    # def get_avg_type(self):                                                     # get avg type
    #     """ get avg type """
    #     return self.query("AVER:TYPE?")

    def set_source_state(self, state):
        """ set source enable on/off """
        if state == 'ON':
            self.write("SOUR:ENAB ON")
        elif state == 'OFF':
            self.write("SOUR:ENAB OFF")
        else:
            print('Warning: Enable State NOT Found!')
        return

    def set_source_power(self, num):
        """ set source power """
        self.write("SOUR:POW %d" % num)
        return

    def set_avg_count(self, num):                                               # set avg count
        """ set avg count """
        self.write("AVER:COUN %d" % num)
        return

    def get_avg_count(self):                                                     # get avg count
        """ get avg count """
        return self.query_int("AVER:COUN?")

    def restart_avg(self):                                                       # restart avg
        """ restart avg """
        # self.write("INIT:REST;*OPC?")
        self.write("INIT:REST")
        return

    def set_trace_number(self, num):
        """ set trace number of points on spectrum """
        self.write("SWE:POIN "+str(num+1))

    def set_channel_measurement_mode(self, mode):
        """ set channel measurement mode as """
        """ CHP: Channel Power """
        """ OBW: Occupied Bandwidth """
        """ ACPR: Adjacent Channel Power """
        """ None: No current channel measurement. """
        if not mode in ['CHP', 'OBW', 'ACPR', 'None']:
            print('Warning: Channel measurement mode not supported!')
        self.write("MEAS:CHAN "+mode)
        return

    def set_channel_power_band(self, band):
        """ set channel power integration bandwidth"""
        self.write("SENS:CME:IBW "+str(band))

    def get_channel_power(self, count=15):
        """ get channel power"""
        """ CHP: Main Channel Power in dBm """
        """ PSD: Main Channel Power Spectral Density (PSD) in dBm/Hz """
        self.set_channel_measurement_mode('CHP')
        self.set_avg_count(count)
        self.restart_avg()
        x = self.query("CALC:MEAS:DATA?")
        CHP = float(x.split(',')[0])
        PSD = float(x.split(',')[1])
        return CHP, PSD

    def set_RF_atten(self, atten):
        self.write("POW:ATT:AUTO 0")
        self.write("POW:ATT "+str(atten))
    
    def save_file(self, fileName='record.csv', device='USBDISK'):
        if device == 'USBDISK':
            self.write("MMEM:CDIR \"[USBDISK]:\"")
        elif device == 'SDCARD':
            self.write("MMEM:CDIR \"[SDCARD]:\"")
        elif device == 'INTERNAL':
            self.write("MMEM:CDIR \"[INTERNAL]:\"")
        else:
            print('Warning: Device NOT Available!')
        self.write("MMEM:STOR:FDAT \""+fileName+"\"")
...
class SA_Keysight_Generic(SA_Generic):
    ...
    def init_meas(self):                                                        # initialize to set up measurement
        ...
        if self.port_open:
            ...
            self.set_span(5e9)                                                  # 5MHz span
            self.set_rbw(1e9)                                                   # 100kHz RBW
            self.set_vbw(1e9)                                                   # 1kHz VBW
            self.set_rlev(-20)                                                  # set RLEV=-20dBm
            self.set_atten(0)                                                   # 0dB attenuation
            self.enable_marker(1)                                               # enable marker1

            print(self.get_marker_freq(1))
            print(self.get_marker(1))
...
class SA_Keysight_EXA(SA_Keysight_Generic):
    ...
    def align_all_now(self, blocking=1):                                        # align all now
        ...
        if blocking:                                                            # blocking calibration - wait until done before returning
            ...
            self.port.set_timeout(60)                                           # set timeout to 60sec
...
class VNA_Anritsu_VectorStar(VNA_Generic):
    ...
    def init_meas(self):                                                        # initialize VNA
        ...
        if self.port_open:
            ...
            self.port.set_timeout(20)                                           # set timeout = 20sec
...
def query_idn(addr):
    ...
    if inst.open_resource(addr):
        try:
            ...
            inst.set_timeout(5)                                                 # set timeout to 5sec
...
driver_list = [
    ['SA',      'KEYSIGHT*',    'N9915*',   SA_Keysight_Generic()],
    ['SA',      'KEYSIGHT*',    'N9951*',   SA_Keysight_Generic()],
    ...
    ['VNA',     'ANRITSU*',     'MS464*',   VNA_Anritsu_VectorStar()],      # Anritsu VectorStar VNA
    ['VNA',     'ANRITSU*',     'MS461*',   VNA_Anritsu_VectorStar()],      # Anritsu ShockLine VNA
    ['VNA',     'ANRITSU*',     'MS463*',   VNA_Anritsu_VectorStar()],      # Anritsu ShockLine VNA
    ['VNA',     'ANRITSU*',     'MS465*',   VNA_Anritsu_VectorStar()],      # Anritsu ShockLine VNA
    ['VNA',     'ANRITSU*',     'ME7868*',  VNA_Anritsu_VectorStar()],      # Anritsu ShockLine Modular 2-port VNA
...
driver_default_list = [
    ...
    ['VNA',     'ANRITSU*',     '*',        VNA_Anritsu_VectorStar()],
