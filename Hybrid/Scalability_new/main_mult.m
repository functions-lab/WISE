...
calib = [ ...
    ...
    "/home/zg88/CEI/Analog/Code/Hybrid/Calibration_new/Cache/channel_300_user2.mat" ...
    "/home/zg88/CEI/Analog/Code/Hybrid/Calibration_new/Cache/channel_300_user3.mat"];
% userNum = 1;
% calib = [ ...
%     "/home/zg88/CEI/Analog/Code/Hybrid/Calibration_new/Cache/channel_300_user1.mat"];
...
resultPath = "Spectrum_"+param.encMode+"-"+param.decMode+"/";
...
for inputSizeIdx = 1: length(inputSizeList)
    ...
    pearMat_sim = NaN(testNum, powerNum);
    rmseMat_sim = NaN(testNum, powerNum);
    pearMat_exp = NaN(testNum, userNum, powerNum);
    rmseMat_exp = NaN(testNum, userNum, powerNum);
    outputDigitMat = NaN(testNum, batchNum, outputSize);
    outputAnalMat_sim = NaN(testNum, powerNum, batchNum, outputSize);
    outputAnalMat_exp = NaN(testNum, userNum, powerNum, batchNum, outputSize);
...
% %         ylabel("Analog Computing");
        ...
        outputAnalMat_exp(testIdx, :, :, :, :) = reshape(outputAnalList_exp, [1 userNum powerNum batchNum outputSize]);
        for userIdx = 1: userNum
            for powerIdx = 1: powerNum
                outputAnal_exp = reshape(outputAnalList_exp(powerIdx, userIdx, :, :), [batchNum outputSize]);
                pear_exp = mean(CalcPearson(abs(outputDigit).', abs(outputAnal_exp).'));
                pearMat_exp(testIdx, userIdx, powerIdx) = pear_exp;
                rmse_exp = mean(CalcRMSE(abs(outputDigit).', abs(outputAnal_exp).'));
                rmseMat_exp(testIdx, userIdx, powerIdx) = rmse_exp;
            end
        end
...
%         ylabel("Analog Computing");
        ...
        hold on;
        plot(emacList_exp, squeeze(median(median(rmseMat_exp, 1, 'omitnan'), 2, "omitnan")), 'x');
        hold off;
        set(gca, 'XScale', 'log');
        xlabel("E_{MAC} J/MAC");
        ...
        hold on;
        plot(emacList_exp, squeeze(median(median(pearMat_exp, 1, 'omitnan'), 2, "omitnan")), 'x');
        hold off;
        set(gca, 'XScale', 'log');
        xlabel("E_{MAC} J/MAC");
        xlim([min(emacList_exp)/2 max(emacList_exp)*2]);
